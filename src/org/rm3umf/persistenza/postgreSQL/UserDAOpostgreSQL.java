...
//						+ "LIMIT 10 ";
	...
	public List<String> doRetriveUsernamesById(long id) throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String retrieve = "select username from new_news_recommendation.username where userid=?";
			statement = connection.prepareStatement(retrieve);
	...
	public void save(User user) throws PersistenceException {
		...
		try {
			...
			String insert = "insert into new_news_recommendation.users(id) values (?)";
	...
	private void saveUsername(long iduser,String username) throws PersistenceException {
		...
		PreparedStatement statement = null;
		try {			
			String insertUsername = "insert into new_news_recommendation.username(userid,username) values (?,?)";
	...
	public List<UserModel> twittomender_tests1() throws PersistenceException {
		List<UserModel> user_model=new LinkedList<UserModel>();
		UserModel um;
		User u;
		DataSourcePostgreSQL ds = DataSourcePostgreSQL.getInstance();
		Connection connection = null;
		...
		try {
			connection = ds.getConnection();
			String retrieve = "select Distinct(id_user), count(*) as t from new_news_recommendation.user_news Group by id_news order by t DESC";
			statement = connection.prepareStatement(retrieve);
			result = statement.executeQuery();
			while(result.next()){
				u = new User();
	...
	public List<UserModel> twittomender_tests7() throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String retrieve = "	SELECT DISTINCT(userid), count(follower) as t FROM new_news_recommendation.follower, user_news where userid=id_user  Group By userid order by t DESC";
			statement = connection.prepareStatement(retrieve);
			result = statement.executeQuery();
			while(result.next()){
				u = new User();
