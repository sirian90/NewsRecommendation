...
public class TweetsTypeEntityDAOpostgreSQL implements TweetsEntityDAO{
	...
	public List<TweetEntity> getTweetEntityFromTweetId(String tweetId, String type) throws PersistenceException {
		...
		try {
			...
			String retrieve = "select userId, tweetId, semanticsTweetsTypeEntity.type, typeURI, semanticsTweetsTypeEntity.name, uri, relevance, creationTime, Count(*) "
					+ "from new_news_recommendation.semanticsTweetsTypeEntity "
					+ "where tweetId=? "
					+ "Group By semanticsTweetsTypeEntity.type";
...
TweetEntity ne = null;
		...
		try {
			...
			String retrieve = "select userId, tweetId, semanticsTweetsTypeEntity.type, typeURI, semanticsTweetsTypeEntity.name, uri, relevance, creationTime, Count(*) "
					+ "from new_news_recommendation.semanticsTweetsTypeEntity "
					+ "where semanticsTweetsTypeEntity.type=?";
			statement = connection.prepareStatement(retrieve);
	...
	public List<Enrichment> filter(int tHRESHOLD) throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String retrieve = "select type, tmpcount "
					+ "from ("
					+ "SELECT type ,COUNT(*) as tmpcount "
					+ "FROM new_news_recommendation.semanticsTweetsTypeEntity "
					+ "GROUP BY type) "
					+ "as tmp "
					+ "where tmpcount < ?";
	...
	public void delete(Enrichment enr) throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String update = "DELETE FROM semanticsTweetsTypeEntity " +
					"WHERE type=?";
	...
	public List<Enrichment> retrieveAll() throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String retrieve = "SELECT semanticsTweetsTypeEntity.type, COUNT(*) FROM new_news_recommendation.semanticsTweetsTypeEntity GROUP BY semanticsTweetsTypeEntity.type;";
			statement = connection.prepareStatement(retrieve);
			result = statement.executeQuery();
			while(result.next()){
				ne = new Enrichment();
	...
	public Map<String, Enrichment> retrieve4UserAndPeriod(User user,
		...
		try {
			connection = ds.getConnection();
			String retrieve = "SELECT semanticsTweetsTypeEntity.type, count(*) "
					+ "FROM new_news_recommendation.semanticsTweetsTypeEntity, new_news_recommendation.tweets_sample "
