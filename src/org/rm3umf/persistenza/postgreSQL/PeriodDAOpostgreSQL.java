...
public class PeriodDAOpostgreSQL implements PeriodDAO{
	...
	public String getMinDate() throws PersistenceException {
		...
		try {
			connection = ds.getConnection();
			String retrieve = "SELECT min(creationTime) from tweets_sample";
			statement = connection.prepareStatement(retrieve);
			result = statement.executeQuery();
			if(result.next()){
	...
	public void save(Period period) throws PersistenceException {
		...
		try {			
			...
			statement.setTimestamp(2,Timestamp.valueOf(period.getDataInizioPeriodo()));
			statement.setTimestamp(3, Timestamp.valueOf(period.getDataFinePeriodo()));
	...
	public String getMaxDate4User(User u) throws PersistenceException {
		String maxDate=null;
		DataSourcePostgreSQL ds = DataSourcePostgreSQL.getInstance();
		Connection connection = null;
		PreparedStatement statement = null;
		...
		try {
			connection = ds.getConnection();
			String retrieve = "SELECT max(date) from message where userid=?";
			statement = connection.prepareStatement(retrieve);
	...
	@Override
	public String getMinDate4User(User u) throws PersistenceException {
		String minDate=null;
		DataSourcePostgreSQL ds = DataSourcePostgreSQL.getInstance();
		Connection connection = null;
		PreparedStatement statement = null;
		...
		try {
			connection = ds.getConnection();
			String retrieve = "SELECT min(date) from message where userid=?";
			statement = connection.prepareStatement(retrieve);
			statement.setLong(1, u.getIduser());
			result = statement.executeQuery();
			if(result.next()){
