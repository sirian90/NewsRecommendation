...
public class TypeDAOpostgreSQL implements TypeDAO{
	...
	public List<Enrichment> getTypeFromTweetId(String tweetId) throws PersistenceException {
		...
		try {
			...
			String retrieve = "select type, Count(*) "
				...
				+"	where tweetId=? " 
				+"	Group By semanticsTweetsEntity.type "
				+"	union all "
				+"	select semanticsNewsEntity.type, Count(*) "
				+"	from new_news_recommendation.semanticsNewsEntity, user_news "
				+"	where id_news=newsId and id_tweet=? " 
				+"	Group By semanticsNewsEntity.type";
