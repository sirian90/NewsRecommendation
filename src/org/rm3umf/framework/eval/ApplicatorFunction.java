 ...
 * Il result contiene i bestUser relativi alla fase di traning del sistema (il 70% del dataset)
 * 
 * @author Sirian
 */
...
//	private List<UserModelWithScore> userModels;
	...
	public Result apply(List<UserModel> userModels){
		...
		for(UserModel umCorr:userModels){
			...
			Map<Long, Double> bestUsers=getBestUser(umCorr,userModels);
	 ...
	 */
	private Map<Long, Double> getBestUser(UserModel umCorr,List<UserModel> listUserModel){
...
//		Collections.sort(listUserModelWithScore,this.similarityFunction);
		listUserModelWithScore = UtilMath.sortListByScore(listUserModelWithScore);
		...
		if(listUserModelWithScore.size() < this.n){
			indexLastElement = listUserModelWithScore.size();
		}
		//Metto gli N con ranking più alto in una lista di Long
		Map<Long, Double> bestUserModel=new HashMap<Long, Double>();
		for(UserModelWithScore um : listUserModelWithScore.subList(0,indexLastElement)){
			...
			double score = um.getScore();
			bestUserModel.put(userid,score); 
		}
