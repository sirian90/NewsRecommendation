...
public class Result {
	...
	private  Map<Long,Map<Long, Double>> result; //rappresenta il risultato da valutare 
	...
	public Result(String functionSimilarity){
		this.result=new HashMap<Long,Map<Long, Double>>();
		this.duration=-1;
	 ...
	 */
	public Result(){
		this.result=new TreeMap<Long,Map<Long, Double>>();
	}
	 ...
	 * @param userBest
	 */
	public void addBestUser(long userid,long userBest, double score){
		Map<Long, Double> listBest = this.result.get(userid);
		if(listBest==null){
			listBest= new HashMap<Long, Double>();
			this.result.put(userid,listBest);
		}
		//aggiungo lo userBest in ultima posizione della lista
		listBest.put(userBest,score);
	}
	 ...
	 * @param bestUsers
	 */
	public void addListBestUser(Long userid,Map<Long, Double> bestUsers_score){
		result.put(userid, bestUsers_score);
	}
	 ...
	 * @return listBestUser
	 */
	public Map<Long,Double> getBestUsers(Long userid){
