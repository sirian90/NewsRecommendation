...
public class VectorUtil {
	...
	public static double cosineSimilarity(double[] s1, double[] s2){
		...
		*/
		...
		//Coseno Similarità di Sirian 	
		
		double numeratore = 0;
		double denominatore = 0;
		double denominatore_parziale_s1 = 0;
		double denominatore_parziale_s2 = 0;
		
		for (int i = 0; i< s1.length; i++) {
			numeratore += s1[i]*s2[i];
			denominatore_parziale_s1 += Math.pow(s1[i], 2);
			denominatore_parziale_s2 += Math.pow(s2[i], 2);
		}
		
		denominatore = Math.sqrt(denominatore_parziale_s1) * Math.sqrt(denominatore_parziale_s2);
